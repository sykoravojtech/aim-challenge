...
| retrieve | `pipeline/retrieval.py::retrieve_relevant_chunks` + `collapse_chunks_by_article` (+ `build_query_text`/`build_query_filter` primitives wired into `main.py::run_pipeline`) | ✅ Phase 0–4, 6G | Hybrid retrieval: `{"region": {"$in": [*aim.regions, "Global"]}}` as a Pinecone filter *before* ANN. Global always OR'd in so Global-tagged pieces still serve regional Aims. **Phase 4**: `include_values=True` so MMR has the vectors. **Phase 6G**: retrieve a wide raw-chunk pool (`RETRIEVE_RAW_K=1000`), collapse to best-scored chunk per `article_id` (`RETRIEVE_TOP_K=40` unique articles). Absorbs Pinecone chunk-duplication from repeated `force` runs — Tier 3 semantic dedup filters `article_id $ne` so same-URL re-upserts accumulate, and one article was holding 126 chunks under the saas filter. Post-fix saas eval: recall 0.00 → 0.17, relevance 3.00 → 4.33. |
